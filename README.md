...
No quotes except where necessary, and no commas required (they're optional, and treated like whitespace). So that's kind of nice. More useful Humon might look like this asset description for a game's material description:
...
Each dict entry must be a key:object pair. The value portion can be any kind of object. Each key in a dict must be a string, and unique within the dict.

**Keys must be strings.**
They can be numbers of course, but that's just a string to Humon. Other language bindings like ES6 might interpret them their own way, in their own sensible context. But as written, a key as a token must be a sequence of value characters, quoted or unquoted.
...
I find myself often using commas when placing list elements on one line, as on line foo above.
...
The APIs can return all of the values above by the `movie` key and an index. Note, this isn't terribly useful or a good idea, as the APIs can be cumbersome. But the nature of metadata is more free-form, so Humon supports it.
...
If this is all Groot to you, just rest assured that any modern text editor or web server or web browser reads and writes UTF8, and can handle foreign language text and emojis, and Humon speaks that encoding too.
...
If you're never searching by comments, or are ignoring them altogether, then none of these rules really matter. But for such a mission, the associations can be handy. IDEs could machine on comments for smart document summaries, for example.
...
*loadPlayer.cpp:*
    ...
    if (trove.getAnnotation("app") != "hudo")
        { throw runtime_error(format("{}: file is not a hudo file.", path)); }

    auto version = trove.getNumAnnotationsByKey("version")
                 ? Version<3> { trove.getAnnotationByKey("version", 0) }
                 : Version<3> { };
