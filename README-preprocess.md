...
!!! src: apps/readmeSrc/usage.cpp; frag: val4; indent: -2
...
!!! src: apps/readmeSrc/usage.cpp; frag: anno1

    ...

!!! src: apps/readmeSrc/usage.cpp; frag: anno2; indent: -2
...
!!! src: apps/readmeSrc/usage.cpp; frag: anno3; indent: -2
...
This will generate an uncolored, well-formatted token stream, or an error code. These are returned as a `std::variant<std::string, hu::ErrorCode>`. The `hu::SerializeOptions` class has formatting options to customize the token stream to your liking.
...
* libhumon.so.!!!HUMON_VERSION!!!        - shared library for Linux
* test                     - test binary
